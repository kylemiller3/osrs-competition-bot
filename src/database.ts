...
import {
    upsertEventStmt, upsertTeamStmt, upsertParticipantStmt, upsertAccountStmt, upsertGuildStmt, upsertMessagesStmt, fetchEventStmt, createTableAccountStmt, createTableEventsStmt, createTableGuildStmt, createTableMessageStmt, createTableParticipantsStmt, createTableTeamsStmt,
} from './databaseMisc';
...
export namespace Db {
    ...
    export const fetchEvent = async (
        ...
        console.log(fetchEventStmt.text);
        ...
        await db.task(
            async (): Promise<void> => {
                const eventId = await db.one(
                    ...
                    [
                        event.name,
                        event.when.start.toISOString(),
                        event.when.end.toISOString(),
                        event.global,
                        event.tracking.category,
                        event.tracking.what === undefined
                            ? null
                            : event.tracking.what.toString(),
                        event.adminLocked,
                    ],
                );
                            ...
                            async (participant: Event.Participant): Promise<void> => {
                                ...
                                Promise<void>[] = participant.runescapeAccounts.map(
                                    async (account: Event.Account): Promise<void> => {
                                        const endingPart = account.ending !== undefined
                                            ? [
                                        ...
                                        const startingPart = account.starting !== undefined
                                            ? [
