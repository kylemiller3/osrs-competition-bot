...
export namespace Db {
  ...
  const rowToEvent = (eventRow: EventRow): Event.Standard => {
  ...
  export const createTables = (
    ...
    async (task: pgp.ITask<unknown>): Promise<void> => {
      ...
      task.none({
        text:
            'CREATE INDEX IF NOT EXISTS idx_tier ON '
            + `${TABLES.SETTINGS}`
            + '('
      ...
      task.none({
        text:
            'CREATE INDEX IF NOT EXISTS idx_start ON '
            + `${TABLES.EVENTS}`
            + '('
            + 'f_cast_isots'
            + '('
      ...
      task.none({
        text:
            'CREATE INDEX IF NOT EXISTS idx_end ON '
            + `${TABLES.EVENTS}`
            + '('
            + 'f_cast_isots'
            + '('
      ...
      task.none({
        text:
            'CREATE INDEX IF NOT EXISTS idx_participants ON '
            + `${TABLES.EVENTS} `
            + 'USING gin '
            + '('
  ...
  export const upsertEvent = async (
    ...
    if (event.id === undefined) {
      const ret: {
        id: number;
        event: Event.Standard;
      } = await db.one(insertNewEventStmt, [event.dbStringify]);
      return rowToEvent(ret);
    }
    const ret: {
      id: number;
      event: Event.Standard;
    } = await db.one(updateEventStmt, [event.id, event.dbStringify]);
    return rowToEvent(ret);
  };
  ...
  export const fetchEvent = async (
    ...
    const ret: EventRow | null = await db.oneOrNone(fetchEventStmt, id);
    if (ret === null) return null;
    return rowToEvent(ret);
  };
  ...
  export const fetchLocallyCreatedEvent = async (
    ...
    const ret: EventRow | null = await db.oneOrNone(fetchCreatorEventStmt, [
      id,
      guildId,
    ]);
    if (ret === null) return null;
  ...
  export const fetchAnyGuildEvent = async (
    ...
    const ret: EventRow | null = await db.oneOrNone(fetchGuildEventStmt, [
      id,
      guildId,
    ]);
    if (ret === null) return null;
  ...
  export const fetchGlobalEvent = async (
    ...
    const ret: EventRow | null = await db.oneOrNone(fetchGlobalEventStmt, id);
    if (ret === null) return null;
    return rowToEvent(ret);
  };
  ...
  export const fetchInvitedEvent = async (
    ...
    const ret: EventRow | null = await db.oneOrNone(fetchInvitedEventStmt, [
      id,
      guildId,
    ]);
    if (ret === null) return null;
  ...
  export const fetchAllEventsBetweenDates = async (
    ...
    const ret: EventRow[] | null = await db.manyOrNone(eventsBetweenDatesStmt, [
      dateA.toISOString(),
      dateB.toISOString(),
    ]);
    if (ret === null) return null;
  ...
  export const fetchAllGuildEventsBetweenDates = async (
    ...
    const ret:
      | EventRow[]
      | null = await db.manyOrNone(guildEventsBetweenDatesStmt, [
        guildId,
        dateA.toISOString(),
        dateB.toISOString(),
      ]);
  ...
  export const deleteEvent = (
    eventId: number,
    db: pgp.IDatabase<unknown> = Db.mainDb,
  ): Promise<null> => db.none(deleteEventStmt, eventId);
  ...
  export const upsertSettings = async (
    ...
    } = await db.one(upsertSettingsStmt, [
      settings.guildId,
      settings.channelId,
    ]);
    return {
  ...
  export const fetchSettings = async (
    ...
    } | null = await db.oneOrNone(fetchSettingsStmt, [guildId]);
    if (ret === null) return null;
    return {
      guildId: ret[SETTINGS_COL.GUILD_ID],
      channelId: ret[SETTINGS_COL.CHANNEL_ID],
