...
import {
  catchError, publishReplay, refCount, mergeMap,
} from 'rxjs/operators';
...
export namespace Network {
  ...
  export const genericNetworkFetch$ = <T>(
    ...
    const networkRequestFactory: Observable<T> = defer(
      (): Observable<T> => of(null).pipe(mergeMap((): Observable<T> => from(bound()))),
    );
  ...
  export const hiscoresFetch$ = (
    ...
    if (cachedRsn === undefined) {
      ...
      const isInputError: (error: Error) => boolean = (error: Error): boolean => error.message === 'Player not found! Check RSN or game mode.'
      ...
      const obs = genericNetworkFetch$<hiscores.Player | null>(
        ...
        catchError(
          (error: Error): Observable<null> => {
            if (isInputError(error)) {
              Utils.logger.debug(
                `The rsn could not be looked up because ${error.message}`,
              );
              // consume this error
      ...
      if (keys.length >= CACHE_SIZE) {
        const idxToRemove: number = Math.floor(Math.random() * CACHE_SIZE);
