...
import { gClient, getTagFromDiscordId, getDiscordGuildName } from '..';
...
export namespace Event {
   ...
   * @category Tracking
   */
  export type BountyHunter = 'rogue' | 'hunter';
  ...
  export class Standard {
    ...
    public addScore(scoreToAdd: number, participantId: string): void {
      ...
      this._teams.forEach((team: Team, idx: number): void => {
    ...
    public signupParticipant(
      ...
      boolean => participant.runescapeAccounts.some(
        (account: Event.Account): boolean => account.rsn.toLowerCase() === rsn.toLowerCase(),
      );

      const rsnIdx: number = this._teams.findIndex(
        (team: Event.Team): boolean => team.participants.some(findRsn),
      );

      const rsnJdx: number = rsnIdx !== -1
        ? this._teams[rsnIdx].participants.findIndex(findRsn)
        : -1;
      ...
      const participantIdx: number = this._teams.findIndex(
        (team: Event.Team): boolean => team.participants.some(
          (participant: Event.Participant): boolean => participant.userId === participantId,
        ),
      );
      ...
      if (participantIdx !== -1 && participantJdx !== -1) {
        ...
        const participant: Event.Participant = this._teams[participantIdx]
          .participants[participantJdx];
        this._teams[participantIdx].participants[
          participantJdx
        ].runescapeAccounts = participant.runescapeAccounts.concat({
          rsn,
        });
        return false;
      }
      ...
      const teamIdx: number = this._teams.findIndex(
        (team: Event.Team): boolean => teamName !== undefined
    ...
    public unsignupParticipant(participantId: string): void {
      ...
      const userIdx: number = this._teams.findIndex(
        (team: Event.Team): boolean => team.participants.some(findUser),
      );
      const userJdx: number = userIdx !== -1
        ? this._teams[userIdx].participants.findIndex(findUser)
        : -1;
      if (userJdx === -1) {
    ...
    public statusString(): string {
      ...
      return `active (${timeLeft.d}d ${padToTwo(timeLeft.h)}h ${padToTwo(
        timeLeft.m,
      )}m remain)`;
    }
    ...
    public async listParticipants(): Promise<string> {
      ...
      const retMsgsResolver: Promise<string>[] = this._teams.map(
        async (team: Event.Team): Promise<string> => {
          const participantResolver: Promise<
            discord.User | string
          >[] = team.participants.map(
            (participant: Event.Participant): Promise<discord.User | string> => gClient
              .fetchUser(participant.userId)
          ...
          const participantStr: string = discordUsers
            .map((user: discord.User | string, idx: number): string => {
              const participant: Event.Participant = team.participants[idx];
              const rsnStrs: string = participant.runescapeAccounts
                .map(
    ...
    public teamsScoreboards(): TeamScoreboard[] {
      ...
      const teamsScores: TeamScoreboard[] | undefined = this._teams
        .map(
          (team: Team): TeamScoreboard => {
            const participantsScores: ParticipantScoreboard[] = team.participants
              .map(
                (participant: Participant): ParticipantScoreboard => {
                  const accountsScores: AccountScoreboard[] = participant.runescapeAccounts
                    .map(
                      (account: Account): AccountScoreboard => {
                        const whatsScores: WhatScoreboard[] | undefined = whatKeys === undefined
                          ? undefined
                          : whatKeys
                            .map(
                              (whatKey: string): WhatScoreboard => {
                                if (
                                  account.ending !== undefined
                                      && account.starting !== undefined
                                ) {
                                  // case of a new boss or skill or something
                                  // we may not have the starting defined
                                  if (
                                    account.ending[categoryKey]
                                          !== undefined
                                    ...
                                    if (categoryKey === 'skills') {
                                      const ending = Math.max(
                                        account.ending.skills[whatKey].xp,
                                        0,
                                      );
                                      return {
                                        lhs: whatKey,
                                  ...
                                  if (categoryKey === 'skills') {
                                    ...
                                    const starting = Math.max(
                                      account.starting.skills[whatKey].xp,
                                      0,
                                    );
                                    return {
                                      lhs: whatKey,
                  ...
                  const participantScore: number = accountsScores
                    .map(
            ...
            const teamScore: number = participantsScores
              .map(
                (participant: ParticipantScoreboard): number => participant.participantScore,
              )
              .reduce(add, 0);
        ...
        .sort(
          (a: TeamScoreboard, b: TeamScoreboard): number => b.teamScore - a.teamScore,
        );
      return teamsScores;
    }
    ...
    public async scoreboardStr(
      ...
      const promises: Promise<string>[] = currentScoreboard.flatMap(
        (team: TeamScoreboard): Promise<string>[] => team.participantsScores.flatMap(
          (participant: ParticipantScoreboard): Promise<string> => {
            ...
            if (displayName === null) {
              return getTagFromDiscordId(gClient, participant.lhs);
            }
      ...
      const getTeamPrefix = (idx: number, lastIdx: number): string => {
        switch (idx) {
          case 0:
            return '🥇 ';
          case 1:
      ...
      let idx = 0;
      const maxTeamsLen: number[] = currentScoreboard
        .map((team: TeamScoreboard, idi: number): number => {
          const participantsLen: number[] = team.participantsScores.flatMap(
            (participant: ParticipantScoreboard): number => {
              const participantLen: number = `${tab}${
                tags[idx]
              }${participant.participantScore.toLocaleString('en-us')}`.length;
              idx += 1;
              const accountsLen: number[] = participant.accountsScores.flatMap(
                (account: AccountScoreboard): number => {
                  const accountLen: number = `${tab}${tab}${
                    account.lhs
                  }${account.accountScore.toLocaleString('en-us')}`.length;
                  if (account.whatsScores !== undefined) {
                    const whatsLen: number[] = account.whatsScores.flatMap(
                      (what: WhatScoreboard): number => {
                        const whatLen: number = `${tab}${tab}${tab}${
                          what.lhs
                        }${what.whatScore.toLocaleString('en-us')}`.length;
                        return whatLen;
                      },
                    );
                    return Math.max(Math.max(...whatsLen), accountLen);
                  }
                  return accountLen;
                },
              );
              return Math.max(Math.max(...accountsLen), participantLen);
            },
          );
          const teamLen: number = team.participantsScores.length > 1
            ? `${getTeamPrefix(idi, currentScoreboard.length - 1)}${
              team.lhs
            }${team.teamScore.toLocaleString('en-us')}`.length
            : `${getTeamPrefix(idi, currentScoreboard.length - 1)}`.length;
          return Math.max(Math.max(...participantsLen), teamLen);
        })
        .map((maxTeamLen: number): number => maxTeamLen + padding);
      ...
      const str: string = `${dashesTeamSeparator}\n`
        .concat(
          currentScoreboard
            .map((team: TeamScoreboard, idi: number): string => {
              ...
              const teamStr = team.participantsScores.length > 1
                ? `${getTeamPrefix(idi, currentScoreboard.length - 1)}${
                  team.lhs
                }${spacesTeam}${team.teamScore.toLocaleString('en-us')}`
                : `${getTeamPrefix(idi, currentScoreboard.length - 1)}`;

              const participantsStr = team.participantsScores
                .map((participant: ParticipantScoreboard): string => {
                  const participantStrLen: number = `${tab}${
                    tags[idx]
                  }${participant.participantScore.toLocaleString('en-us')}`
                    .length;
                  const spacesToInsertParticipant: number = maxTeamStrLen - participantStrLen;
                  const spacesParticipant: string = new Array(
                    spacesToInsertParticipant + 1,
                  ).join(' ');
                  const participantStr = `${tab}${
                    tags[idx]
                  }${spacesParticipant}${participant.participantScore.toLocaleString(
                    'en-us',
                  )}`;
                  idx += 1;

                  const accountsStr: string = participant.accountsScores
                    .map((account: AccountScoreboard): string => {
                      const accountStrLen: number = `${tab}${tab}${
                        account.lhs
                      }${account.accountScore.toLocaleString('en-us')}`.length;
                      const spacesToInsertAccount: number = maxTeamStrLen - accountStrLen;
                      const spacesAccount: string = new Array(
                        spacesToInsertAccount + 1,
                      ).join(' ');
                      const accountStr = `${tab}${tab}${
                        account.lhs
                      }${spacesAccount}${account.accountScore.toLocaleString(
                        'en-us',
                      )}`;

                      if (account.whatsScores !== undefined) {
                        const whatStr: string = account.whatsScores
                          .map((what: WhatScoreboard): string | null => {
                            const whatStrLen: number = `${tab}${tab}${tab}${
                              what.lhs
                            }${what.whatScore.toLocaleString('en-us')}`.length;
                            const spacesToInsertWhat: number = maxTeamStrLen - whatStrLen;
                            const spacesWhat: string = new Array(
                              spacesToInsertWhat + 1,
                            ).join(' ');
                            const ret: string | null = what.whatScore > 0
                              ? `${tab}${tab}${tab}${
                                what.lhs
                              }${spacesWhat}${what.whatScore.toLocaleString(
                                'en-us',
                              )}`
                              : null;
                            return ret;
      ...
      const status: string = !deleted ? this.statusString() : '(DELETED EVENT)';
      return `Event ${this._name} (${this._tracking.category})\n#${
        this._id
      } ${this._when.start.toUTCString()} ${status}\n\n${str}`;
    }
  }
  ...
  export class Global extends Standard {
    ...
    public constructor(
      ...
      super(id, name, start, end, guilds, teams, tracking, global, locked);
    ...
    public signupParticipant(
      ...
      const processedTeamName = teamIdx !== -1 ? this._teams[teamIdx].name : teamName;
    ...
    public async scoreboardStr(
      ...
      const scoreboard: string = await super.scoreboardStr(guildId, deleted);

      const competitors: string = this.guilds
        .map((guild: Event.Guild): string => {
