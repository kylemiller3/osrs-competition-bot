...
class EventsSignupConversation extends Conversation {
  ...
  public async init(): Promise<boolean> {
    ...
    dummy.content = team === undefined ? this._opMessage.author.id : team;
    await this.consumeQa({
      questions: [],
      answer: dummy,
    });
    ...
    switch (this._state) {
      ...
      case CONVERSATION_STATE.Q1E: {
        ...
        if (guildEvent.isGlobal === true) {
          ...
          tenMinutesBeforeStart.setMinutes(
            tenMinutesBeforeStart.getMinutes() - 10,
          );
      ...
      case CONVERSATION_STATE.Q2E: {
        ...
        const rsnJdx: number = rsnIdx !== -1
          ? this._event.teams[rsnIdx].participants.findIndex(findRsn)
          : -1;
        ...
        const participantIdx: number = this._event.teams.findIndex(
          (team: Event.Team): boolean => team.participants.some(
            (participant: Event.Participant): boolean => participant.userId === qa.answer.author.id,
          ),
        );
        ...
        if (participantIdx !== -1 && participantJdx !== -1) {
          ...
          const participant: Event.Participant = this._event.teams[
            participantIdx
          ].participants[participantJdx];
          this._event.teams[participantIdx].participants[
            participantJdx
          ...
          const teamIdx: number = this._event.teams.findIndex(
            (team: Event.Team): boolean => this._teamName !== undefined
          ...
          if (teamIdx === -1) {
            ...
            this._event.teams = [...this._event.teams, team];
          }
      ...
      case CONVERSATION_STATE.Q3E: {
        const teamName: string = this._teamName !== undefined ? this._teamName : qa.answer.content;
        ...
        // we either add a new team or we add to the found team
        const teamIdx: number = this._event.teams.findIndex(
          (team: Event.Team): boolean => team.name.toLowerCase() === teamName.toLowerCase(),
        );
...
const eventsSignup = (msg: discord.Message): void => {
  const params: Command.EventsSignup = Command.parseParameters(
    Command.ALL.EVENTS_SIGNUP,
    msg.content,
  );

  const eventsSignupConversation = new EventsSignupConversation(msg, params);
  ConversationManager.startNewConversation(msg, eventsSignupConversation);
};
