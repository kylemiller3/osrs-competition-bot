...
rateThrottle
  .pipe(
    groupBy((eventAndMessage: [Event.Standard, discord.Message]): number => {
    ...
    mergeMap(
      (
        ...
        tap((eventAndMessage: [Event.Standard, discord.Message]): void => {
          Utils.logger.debug('force update sending');
          MessageWrapper.sendMessage({
            message: eventAndMessage[1],
            content:
                'Forcing update... The scoreboard will be updated when done.',
          });

          willUpdateScores$.next([eventAndMessage[0], true]);
        }),
      ),
    ),
  )
    ...
    switch (this._state) {
      ...
      case CONVERSATION_STATE.Q1E: {
        ...
        if (Number.isNaN(eventId)) {
          ...
          const event: Event.Standard | null = creatorEvent !== null ? creatorEvent : invitedEvent;
...
const forceUpdate = (msg: discord.Message): void => {
  ...
  ConversationManager.startNewConversation(msg, eventsForceUpdateConversation);
};
