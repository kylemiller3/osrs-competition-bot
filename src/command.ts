...
export namespace Command {
  ...
  interface AccessControl {
    controlFunction: (admin: boolean) => boolean;
    description: string;
  }
  ...
  const onlyAdmin: AccessControl = {
    controlFunction: (admin: boolean): boolean => admin,
  ...
  const eventsAdd: Description = {
    ...
    params: {
      ...
      type: {
        ...
        usage:
          '(comma separated lists) skills AND skill1 skill2... OR bh OR lms OR clues AND difficulty1 difficulty2... OR custom OR bosses AND boss list...',
        expectedType: ParamType.STRING,
        required: false,
        default: 'custom',
      },
...
//   };
  ...
  const unjoinGlobal: Description = {
    description:
      "Removes and forcefully removes your Guild's participants from a global event.",
    accessControl: onlyAdmin,
    command: '.gleave',
    params: {
      id: {
   ...
   * @category Parsing Interfaces
   */
  export interface AdminSetChannel
    extends Record<string, string | number | boolean> {
    channel: string;
  }
  ...
  const adminSetChannel: Description = {
    description:
      'Sets the announcements channel to the first channel mentioned.',
    accessControl: onlyAdmin,
    command: '.setchannel',
    params: {
      channel: {
   ...
   * @category API
   */
  export const getCommandString = (command: Command.ALL): string => lookup[command].command;
   ...
   * @category API
   */
  export const hasAccess = (command: Command.ALL, admin: boolean): boolean => {
    const cDescription: Description = lookup[command];
    const access = cDescription.accessControl.controlFunction(admin);
    return access;
  };
   ...
   * @category API
   */
  export const isValid = (command: Command.ALL, str: string): boolean => {
    const valid: boolean = str
      .toLowerCase()
      .startsWith(lookup[command].command);
    return valid;
  };
   ...
   * @category API
   */
  export const generateCommandUsageString = (command: Command.ALL): string => {
    const cDescription: Description = lookup[command];
    const usage = 'Usage: ';
    const commandStr: string = usage.concat(cDescription.command).concat(' ');
   ...
   * @category Helper
   */
  export const generateHelpString = (admin: boolean): string => {
    const allKeys = Object.keys(lookup);
    const filteredKeys: string[] = allKeys.filter((key: string): boolean => {
      if (admin) return true;
      return lookup[key].accessControl === anyUser;
    });

    const generateSubString = (key: string): string => {
      ...
      const paramStr: string = paramNames
      ...
      return `${cDescription.command}\n\t${
        cDescription.description
      }\n${paramStr}\n\t${generateCommandUsageString(
        (key as unknown) as ALL,
      )}\n`;
    };

    const helpStr: string = filteredKeys
      .map((key: string): string => generateSubString(key))
      .join('\n');
    return helpStr;
  };
   ...
   */
  export const parseParameters = <T>(command: Command.ALL, str: string): T => {
    const cDescription: Description = lookup[command];
    ...
    const regexes: Record<string, RegExp> = allKeys
      .map(
        (anchor: string): Record<string, RegExp> => {
          const filteredEndingAnchors: string[] = allKeys.filter(
            (endingAnchor: string): boolean => endingAnchor !== anchor,
          );
          const endingAnchorRegex: string = filteredEndingAnchors.join('=|');
          return {
            [anchor]: RegExp(
              `(?<=${anchor}=)(.*?)(?:${endingAnchorRegex}=|$)`,
              'i',
            ),
          };
        },
    ...
    const results: Record<string, string | undefined> = allKeys
      .map(
        (key: string): Record<string, string | undefined> => {
          const exec: RegExpExecArray | null = regexes[key].exec(str);
          if (exec === null) {
            if (
              cDescription.params !== undefined
    ...
    const parsed: Record<string, string | number | boolean> = allKeys
      .map(
