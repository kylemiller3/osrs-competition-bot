 ...
 * @category Observable
 */
export const willUpdateScores$: Subject<[
  Event.Standard,
  boolean
]> = new Subject();
/**
...
export const isAdmin = (
  ...
  if (discord.Permissions.FLAGS.ADMINISTRATOR) {
    return (
      guildMember.permissions.has(discord.Permissions.FLAGS.ADMINISTRATOR)
      || guildMember.roles.some(
        (role: discord.Role): boolean => role.name.toLowerCase() === 'osrs event manager',
      )
    );
  }
  return false;
...
export const getDiscordGuildName = (
  ...
  const guild: discord.Guild | undefined = gClient.guilds.get(guildId);
  return guild !== undefined ? `${guild.name}` : null;
};
...
export const getTextChannelFromId = (
  ...
  const channel: discord.Channel | undefined = guild.channels.get(channelId);
...
export const getTagFromDiscordId = async (
  ...
  const user: discord.User | null = await client
    .fetchUser(discordId)
    .catch((): null => null);
  if (user === null) {
    return discordId;
...
export const getDisplayNameFromDiscordId = (
  ...
  const guild: discord.Guild = client.guilds.get(guildId) as discord.Guild;
  if (guild === undefined || !guild.available) return null;
  const foundMember: discord.GuildMember = guild.members.find(
    (member: discord.GuildMember): boolean => member.id === discordId,
  );
...
const commandDispatch$: Observable<discord.Message> = mergedMessage$.pipe(
  filter((msg: discord.Message): boolean => msg.guild && msg.guild.available),
  tap((msg: discord.Message): void => {
    if (msg.content.toLowerCase().startsWith('.exit')) {
      ConversationManager.stopConversation(msg);
    }

    const validCommandKeys: string[] = Object.keys(Command.ALL)
      .filter((key: string): boolean => Number.isNaN(Number(key)))
      .filter((key: string): boolean => Command.isValid(Command.ALL[key], msg.content));

    if (validCommandKeys.length > 0) {
      if (
        !Command.hasAccess(
          Command.ALL[validCommandKeys[0]],
          isAdmin(msg.guild, msg.author),
        )
      ) {
        return;
      }
      ...
      switch (Command.ALL[validCommandKeys[0]]) {
        ...
        default:
          Utils.logger.error(
            `Unhandled command ${Command.ALL[validCommandKeys[0]]}`,
          );
          break;
      }
    }
...
export const spoofMessage = (
  ...
  const content = `${Command.getCommandString(
    newCommand,
  )}${sourceMessage.content.replace(/<@!?[0-9]+>/gi, '')}`;
  ...
  messageInjector$.next(newMessage);
};
...
const resumeRunningEvents = async (): Promise<void> => {
  const events:
    | Event.Standard[]
    | null = await Db.fetchAllCurrentlyRunningEvents();
  if (events === null) {
    return;
  }
  events.forEach((event: Event.Standard): void => {
...
const deleteMessages = async (
  ...
  // get message objects
  const discordMessagesPromises: Promise<discord.Message | null>[] = eventMessage.messageId.map(
    (messageId: string): Promise<discord.Message | null> => channel
  ...
  return Promise.all(
    validDiscordMessages.map(
      (message: discord.Message): Promise<MessageWrapper.Response> => MessageWrapper.deleteMessage({
        message,
      }),
    ),
  );
...
const refreshMessage = async (
  ...
  let deleteMessagePromise: Promise<
    MessageWrapper.Response[] | null
  > = Promise.resolve(null);
  if (oldMessage !== undefined) {
    deleteMessagePromise = deleteMessages(guild, oldMessage);
  }

  let messagePromise: Promise<MessageWrapper.Response | null> = Promise.resolve(
    null,
  );
...
const saveAndNotifyUpdatedEventScoreboard = (
  ...
  const observables: Observable<Event.ChannelMessage | null>[] = event.guilds.map(
    (eventGuild: Event.Guild): Observable<Event.ChannelMessage | null> => {
      ...
      const deferredRefreshScoreboardObservable: Observable<Event.ChannelMessage | null> = defer(
        (): Observable<Event.ChannelMessage | null> => from(
          event.scoreboardStr(eventGuild.guildId, false),
        ).pipe(
          mergeMap(
            (
  ...
  const ret: Observable<Event.Standard> = concat(observables).pipe(
    combineAll(),
    map(
      (channelMessages: (Event.ChannelMessage | null)[]): Event.Standard => {
        channelMessages.forEach(
          (channelMessage: Event.ChannelMessage, idx: number): void => {
    ...
    mergeMap(
      (eventToSave: Event.Standard):
      Observable<Event.Standard> => from(Db.upsertEvent(eventToSave)),
    ),
    tap((savedEvent: Event.Standard): void => {
...
const scheduleEvents = async (): Promise<void> => {
  ...
  events.forEach((event: Event.Standard): void => {
    ...
    if (
      ...
      startTimers[event.id] = setTimeout(async (): Promise<void> => {
        if (event.id !== undefined) {
          const updatedEvent: Event.Standard | null = await Db.fetchEvent(
            event.id,
          );
    ...
    if (
      ...
      endTimers[event.id] = setTimeout(async (): Promise<void> => {
        if (event.id !== undefined) {
          const updatedEvent: Event.Standard | null = await Db.fetchEvent(
            event.id,
          );
  ...
  .pipe(
    map((event: Event.Standard): void => {
      ...
      if (!event.isCustom) {
        ...
        willUpdateScores$.next([event, true]);
      }
    ...
    concatMap(
      (obj: [Event.Standard, boolean]): Observable<Event.Standard> => {
        ...
        if (Utils.isInFuture(event.when.start)) {
          return saveAndNotifyUpdatedEventScoreboard(event, undefined);
        }
        ...
        const flattenedAccounts: Event.Account[] = flattenedParticipants.flatMap(
          (participant: Event.Participant): Event.Account[] => participant.runescapeAccounts,
        );

        const observables: Observable<hiscores.Player | null>[] = flattenedAccounts.flatMap(
          (account: Event.Account):
          Observable<hiscores.Player | null> => Network.hiscoresFetch$(account.rsn, forced).pipe(
            tap((): void => Utils.logger.debug(`Successfully updated rsn ${account.rsn}`)),
            catchError(
              (error: Error): Observable<null> => {
                Utils.logger.warn(
                  `Failed to update rsn ${account.rsn}: ${error.message}`,
                );
                throw error;
              },
            ),
        ...
        // un-flatmap
        let idx = 0;
        const inner: Observable<Event.Standard> = forkJoin(observables)
          .pipe(
            concatMap(
              (
                ...
                const newTeams: Event.Team[] = newEvent.teams.map(
                  (team: Event.Team): Event.Team => {
                    ...
                    const newParticipants: Event.Participant[] = newTeam.participants.map(
                      (participant: Event.Participant): Event.Participant => {
                        const newParticipant: Event.Participant = {
                          ...participant,
                        };
                        const newAccounts: Event.Account[] = newParticipant.runescapeAccounts.map(
                ...
                return saveAndNotifyUpdatedEventScoreboard(newEvent, undefined);
              },
            ),
            tap((savedEvent: Event.Standard): void => {
              didUpdateScores$.next(savedEvent);
          ...
          .pipe(
            catchError(
              (error: Error):
              Observable<Event.Standard> => saveAndNotifyUpdatedEventScoreboard(event, error),
            ),
          );
        return inner;
      },
...
didStartEvent$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} did start.`);
});

willEndEvent$.subscribe((event: Event.Standard): void => {
...
didEndEvent$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} did end.`);
});

didUpdateScores$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} scores did update.`);
});

willAddEvent$.subscribe((event: Event.Standard): void => {
  ...
  if (event.isCustom) {
    ...
    willUpdateScores$.next([event, false]);
  }

  if (!Utils.isInPast(event.when.start) && !Utils.isInPast(event.when.end)) {
    scheduleEvents();
    // print the board
    willUpdateScores$.next([event, false]);
  }
...
didAddEvent$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} has been added.`);
});

willSignUpPlayer$.subscribe((event: Event.Standard): void => {
...
didSignupPlayer$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} did signup player.`);
});

willUnsignupPlayer$.subscribe((event: Event.Standard): void => {
...
didUnsignupPlayer$.subscribe((event: Event.Standard): void => {
  Utils.logger.info(`Event ${event.id} did unsignup player.`);
});

willDeleteEvent$.subscribe((event: Event.Standard): void => {
  ...
  event.guilds.forEach(
    async (guild: Event.Guild): Promise<void> => {
      ...
      const content: string = await event.scoreboardStr(guild.guildId, true);
  ...
  didDeleteEvent$.next(event);
});

didDeleteEvent$.subscribe((event: Event.Standard): void => {
...
const init = async (): Promise<void> => {
  ...
  // tick tock
  setInterval(scheduleEvents, 1000 * 60 * 60 * 24);
  ...
  setInterval(async (): Promise<void> => {
    ...
    autoUpdateEvents.forEach((autoEvent: Event.Standard): void => {
      willUpdateScores$.next([autoEvent, false]);
    });
  }, 1000 * 60 * 15);
  ...
  socks
    .createServer((info, accept, deny): void => {
      gSshClient
        .on('ready', (): void => {
          gSshClient.forwardOut(
            info.srcAddr,
            info.srcPort,
            info.dstAddr,
            info.dstPort,
